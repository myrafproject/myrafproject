...
class Client(object):
    ...
    def __init__(self,
        ...
        self.session = None
        self.apiurl = apiurl
        self.logger = env.Logger(verb=True, debugger=False)
    ...
    def send_request(self, service, args={}, file_args=None):
        ...
        self.logger.log('Python: {}'.format(args))
        json = python2json(args)
        self.logger.log('Sending json: {}'.format(json))
        url = self.get_url(service)
        self.logger.log('Sending to URL: {}'.format(url))
        ...
        if file_args is not None:
            ...
            self.logger.log('Sending form data: {}'.format(data))
            data = urlencode(data)
            data = data.encode('utf-8')
            self.logger.log('Sending data: {}'.format(data))
            headers = {}
        ...
        try:
            f = urlopen(request)
            txt = f.read()
            self.logger.log('Got json: {}'.format(txt))
            result = json2python(txt)
            self.logger.log('Got result:result'.format())
            stat = result.get('status')
            self.logger.log('Got status: {}'.format(stat))
            if stat == 'error':
                errstr = result.get('errormessage', '(none)')
                raise RequestError('server error message: ' + errstr)
            return result
        except HTTPError as e:
            self.logger.log('HTTPError {}'.format(e))
            txt = e.read()
            open('err.html', 'wb').write(txt)
            self.logger.log('Wrote error text to err.html')

    def login(self, apikey):
        args = { 'apikey' : apikey }
        result = self.send_request('login', args)
        sess = result.get('session')
        self.logger.log('Got session: {}'.format(sess))
        if not sess:
    ...
    def _get_upload_args(self, **kwargs):
        ...
        self.logger.log('Upload args: {}'.format(args))
        return args
    ...
    def upload(self, fn=None, **kwargs):
        ...
        if fn is not None:
            try:
                ...
                self.logger.log('File {} does not exist'.format(fn))
                raise
    ...
    def overlay_plot(self, service, outfn, wcsfn, wcsext=0):
        ...
        self.logger.log('Result status: {}'.format(result['status']))
        plotdata = result['plot']
        plotdata = base64.b64decode(plotdata)
        open(outfn, 'wb').write(plotdata)
        self.logger.log('Wrote {}'.format(outfn))
    ...
    def job_status(self, job_id, justdict=False):
        ...
        if stat == 'success':
            ...
            self.logger.log('Calibration: {}'.format(result))
            result = self.send_request('jobs/%s/tags' % job_id)
            self.logger.log('Tags: {}'.format(result))
            result = self.send_request('jobs/%s/machine_tags' % job_id)
            self.logger.log('Machine Tags: {}'.format(result))
            result = self.send_request('jobs/%s/objects_in_field' % job_id)
            self.logger.log('Objects in field: {}'.format(result))
            result = self.send_request('jobs/%s/annotations' % job_id)
            self.logger.log('Annotations: {}'.format(result))
...
if __name__ == '__main__':
    logger = env.Logger(verb=True, debugger=False)
    logger.log("Running with args {}".format(sys.argv))
    import optparse
    ...
    if opt.apikey is None:
        parser.print_help()
        logger.log()
        logger.log('You must either specify --apikey or set AN_API_KEY')
        sys.exit(-1)
    ...
    if opt.upload or opt.upload_url or opt.upload_xy:
        ...
        stat = upres['status']
        if stat != 'success':
            logger.log('Upload failed: status {}'.format(stat))
            logger.log(upres)
            sys.exit(-1)
    ...
    if opt.wait:
        if opt.solved_id is None:
            if opt.sub_id is None:
                logger.log("Can't --wait without a submission id or job id!")
                sys.exit(-1)

            while True:
                stat = c.sub_status(opt.sub_id, justdict=True)
                logger.log('Got status: {}'.format(stat))
                jobs = stat.get('jobs', [])
                if len(jobs):
                    for j in jobs:
                        if j is not None:
                            break
                    if j is not None:
                        logger.log('Selecting job id {}'.format(j))
                        opt.solved_id = j
                        break
                time.sleep(5)
        ...
        while True:
            ...
            logger.log('Got job status: {}'.format(stat))
    ...
    if opt.solved_id:
        ...
        for url,fn in retrieveurls:
            logger.log('Retrieving file from {} to {}'.format(url, fn))
            f = urlopen(url)
            txt = f.read()
            w = open(fn, 'wb')
            w.write(txt)
            w.close()
            logger.log('Wrote to {}'.format(fn))
    ...
    if opt.sub_id:
        logger.log(c.sub_status(opt.sub_id))
    if opt.job_id:
        logger.log(c.job_status(opt.job_id))

    if opt.jobs_by_tag:
        tag = opt.jobs_by_tag
        logger.log(c.jobs_by_tag(tag, None))
    if opt.jobs_by_exact_tag:
        tag = opt.jobs_by_exact_tag
        logger.log(c.jobs_by_tag(tag, 'yes'))

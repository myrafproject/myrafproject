...
Performs a division operation on the ``Fits`` object.

------------

.. method:: Fits.div(other, output=None, override=False) -> Self

    Performs a division operation on the ``Fits`` object.

    **Notes**

    This method can divide numeric values or another ``Fits`` object.
    ...
    **Returns**
        ...
        ``Fits``
            A new ``Fits`` object representing the result of the division.
...
.. code-block:: python
    ...
    new_fits_1 = fits.div(120)
    new_fits_2 = fits / 120

    new_fits_3 = fits.div(other)

...
class cosmicsimage:
    ...
    def __init__(self, rawarray, pssl=0.0, gain=2.2, readnoise=10.0, sigclip = 5.0, sigfrac = 0.3, objlim = 5.0, satlevel = 50000.0, verbose=True, debugger=False):
        """
        ...
        self.Logger = env.Logger(verb=self.verbose, debugger=self.debugger)
    ...
    def labelmask(self, verbose = None):
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Labeling mask pixels ...")
        ...
        if verbose:
            #print("Labeling done")
            self.Logger.log("Cosmic: Labeling done")
    ...
    def clean(self, mask = None, verbose = None):
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Cleaning cosmic affected pixels ...")
        ...
        for cosmicpos in cosmicindices:
            ...
            # i.e. no good pixels : Shit, a huge cosmic, we will have to improvise ...
                ...
                self.Logger.log("Cosmic: OH NO, I HAVE A HUUUUUUUGE COSMIC !!!!!")
            ...
            if verbose:
                #print("Cleaning done")
                #self.Logger.log("Cosmic: Cleaning done")
                pass
    ...
    def findsatstars(self, verbose = None):
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Detecting saturated stars ...")
            # DETECTION
            ...
            if verbose:
                ...
                self.Logger.log("Cosmic: Building mask of saturated stars ...")
            ...
            if verbose:
                ...
                self.Logger.log("Cosmic: We have {} saturated stars.".format(nsat))
            ...
            for i in range(1,nsat+1): # we go through the islands of saturated pixels
                ...
                if verbose:
                    ...
                    self.Logger.log("Cosmic: Mask of saturated stars done")
    ...
    def getsatstars(self, verbose = None):
        ...
        if self.satstars == None:
            self.findsatstars(verbose = self.verbose)
    ...
    def lacosmiciteration(self, verbose = None):
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Convolving image with Laplacian kernel ...")
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Creating noise model ...")
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Calculating Laplacian signal to noise ratio ...")
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Selecting candidate cosmic rays ...")
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic:   {} candidate pixels".format(nbcandidates))
        ...
        if self.satstars is not None:
            if verbose:
                ...
                self.Logger.log("Cosmic: Masking saturated stars ...")
            ...
            if verbose:
                ...
                self.Logger.log("Cosmic:   {} candidate pixels not part of saturated stars".format(nbcandidates))
            
        if verbose:
            #print("Building fine structure image ...")
            self.Logger.log("Cosmic: Building fine structure image ...")
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Removing suspected compact bright objects ...")
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic:   {} remaining candidate pixels".format(nbcosmics))
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic: Finding neighboring pixels affected by cosmic rays ...")
        ...
        if verbose:
            ...
            self.Logger.log("Cosmic:   {} pixels detected as cosmics".format(nbfinal))
    ...
    def run(self, maxiter = 4, verbose = True):
        ...
        self.Logger.log("Cosmic: Starting {} L.A.Cosmic iterations ...".format(maxiter))
        for i in range(1, maxiter+1):
            #print("Iteration %i" % i)
            self.Logger.log("Cosmic: Iteration {}".format(i))
            
            iterres = self.lacosmiciteration(verbose=self.verbose)
            #print("%i cosmic pixels (%i new)" % (iterres["niter"], iterres["nnew"]))
            self.Logger.log("Cosmic: {} cosmic pixels ({} new)".format(iterres["niter"], iterres["nnew"]))
...
def fromfits(infilename, hdu = 0, verbose = True):
    ...
    if verbose :
        ...
        logger.log("Cosmic: FITS import shape : ({}, {})".format(pixelarrayshape[0], pixelarrayshape[1]))
        #print("FITS file BITPIX : %s" % (hdr["BITPIX"]))
        logger.log("Cosmic: FITS file BITPIX : {}".format(hdr["BITPIX"]))
        #print("Internal array type :", pixelarray.dtype.name)
        logger.log("Cosmic: Internal array type : {}".format(pixelarray.dtype.name))
...
def tofits(outfilename, pixelarray, hdr = None, verbose = True):
    ...
    if verbose :
        ...
        logger.log("FITS export shape : ({}, {})".format(pixelarrayshape[0], pixelarrayshape[1]))
    ...
    if verbose :
        ...
        logger.log("Cosmic: Wrote {}".format(outfilename))
